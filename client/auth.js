...
// Logout state flag (global-safe)
window.isLoggingOut = false;

// =====================
// Firebase load detection
// =====================
if (typeof firebase === 'undefined') {
  console.warn('Firebase SDK not detected on initial load');
  window.addEventListener('load', () => {
...
function showError(message, type = 'error') {
  ...
  el.style.color = type === 'success' ? 'green' : 'orange';
...
async function initializeFirebase() {
  ...
  initializationPromise = (async () => {
    ...
    // Firestore persistence (COMPAT CORRECT WAY)
...
function setupAuthStateListener() {
  ...
  authStateUnsubscribe = auth.onAuthStateChanged((user) => {
    ...
    const now = Date.now();
    if (isHandlingRedirect || now - lastRedirect < 1500) return;
...
async function handleSignup(e) {
  ...
  if (!firstName || !lastName) return showError("Enter full name");
  ...
  try {
    ...
    showError("Signup successful! Check email.", "success");
...
async function handleLogout(e) {
  ...
  try {
    ...
    const url = new URL('/index', window.location.origin);
    url.searchParams.set('logout', 'error');
